...
type Dataset = {
  ...
  ml?: {
    anomaly_scores?: Record<string, any>;
    explanations?: any[];
    report?: any;
    spatial?: any;
    spatial_summary?: any;
...
async function api(
  ...
  console.log("[MetalSense API]", options.method || "GET", url);
  ...
  try {
    ...
    console.error("[MetalSense API] Network error", {
      url,
      error: networkError,
    });
    throw new Error(
      `Unable to reach MetalSense backend at ${url}. ${networkError?.message || "Network request failed."}`,
    );
  }
    ...
    (row, index) => {
      ...
      return {
        ...
        id: sampleIdFor(row, index),
                                     ...
                                     date:
                                     sampleDateFor(row) || null,

                                     season:
                                     findField(row, ["season", "sampling_season", "sample_season"]) || null,

                                     dateInferred:
                                     Boolean(row?.date_inferred),
...
function SampleRow({
  ...
  return (
    ...
    {item.date ? prettyDate(String(item.date)) : "Date not provided"}
    {item.season ? ` · ${item.season}` : ""}
    {item.dateInferred ? " · date inferred" : ""}
    </Text>
    </View>
...
function ImportScreen({
  ...
  const importFile = async (
    ...
    try {
      ...
      console.log("[MetalSense Import] Selected file", {
        name: asset.name,
        uri: asset.uri,
      ...
      const importedDataset = await api(
        "/datasets/import",
        {
          method: "POST",
          body: formData,
      ...
      console.log("[MetalSense Import] Backend import succeeded", {
        datasetId: importedDataset?.dataset_id,
        filename: importedDataset?.filename,
        recordCount: importedDataset?.records?.length,
        hasML: Boolean(importedDataset?.ml),
      });
...
function SpatialScreen({
  ...
  useEffect(() => {
    ...
    console.log("[MetalSense Map] Loading map data", dataset.dataset_id);
    ...
    .then(([points, summary, hotspots]) => {
      console.log("[MetalSense Map] Backend data", {
        points: points?.count ?? points?.points?.length ?? 0,
        center: summary?.center,
        hotspots: hotspots?.count ?? hotspots?.hotspots?.length ?? 0,
      });
      if (!active) return;
      setMapData(points);
    ...
    const renderMap = async () => {
      try {
        ...
        if (!first || !Number.isFinite(Number(first.latitude)) || !Number.isFinite(Number(first.longitude))) {
          throw new Error("Map data did not contain a valid latitude/longitude point.");
        }
  ...
  return (
    ...
    {points.length === 0 ? (
      ...
      points.map((point) => (
        <View
        style={styles.sampleRow}
        key={`point-${point.sample_id}-${point.latitude}-${point.longitude}`}
        >
        <View
        style={[
          styles.statusDot,
  ...
  "Not available";
      ...
      const exportPdfSummary = async () => {
        ...
        .map(
          ...
          <td>${item.hei.toFixed(2)}</td>
          <td>${item.cd.toFixed(2)}</td>
          <td style="font-weight:800;color:${riskColor(item.status)}">
        ...
        <strong>${escapeHtml(user.account_type || "Not provided")}</strong>
        </div>

        <div>
        <span>ROLE</span>
        <strong>${escapeHtml(user.role || "Not provided")}</strong>
        </div>

        <div>
        <span>ORGANIZATION</span>
        <strong>${escapeHtml(user.organization || "Not provided")}</strong>
        </div>

        <div>
        <span>INSTITUTION</span>
        <strong>${escapeHtml(user.institution || "Not provided")}</strong>
        </div>

        <div>
        <span>RESEARCH AREA</span>
        <strong>${escapeHtml(user.research_area || "Not provided")}</strong>
        </div>

        <div>
        <span>ACCOUNT CREATED</span>
        <strong>${escapeHtml(prettyDate(user.created_at))}</strong>
        </div>
        </div>
        </div>
        `;
        ...
        <div>
        <span>FILE</span>
        <strong>${escapeHtml(selectedDataset.filename)}</strong>
        </div>

        <div>
        <span>IMPORTED</span>
        <strong>${escapeHtml(prettyDate(selectedDataset.imported_at))}</strong>
        </div>

        <div>
        <span>DATA SOURCE</span>
        <strong>${escapeHtml(selectedDataset.data_source || "Not provided")}</strong>
        </div>

        <div>
        <span>LABORATORY / ORGANIZATION</span>
        <strong>${escapeHtml(selectedDataset.laboratory_organization || "Not provided")}</strong>
        </div>

        <div>
        <span>REPORT ID</span>
        <strong>${escapeHtml(selectedDataset.report_id || "Not provided")}</strong>
        </div>

        <div>
        <span>ANALYTICAL METHOD</span>
        <strong>${escapeHtml(selectedDataset.analytical_method || "Not provided")}</strong>
        </div>

        <div>
        <span>DETECTION LIMIT</span>
        <strong>${escapeHtml(selectedDataset.detection_limit || "Not provided")}</strong>
        </div>

        <div>
        <span>STANDARD</span>
        <strong>${escapeHtml(standard)}</strong>
        </div>
        </div>
        `;
        ...
        * {
        ...
        <section class="page">
        <div class="headline">
        <h1>Heavy Metal Pollution Intelligence</h1>
        </div>
        ...
        <img
        src="${drawRiskDistribution}"
        alt="Risk distribution chart"
        />
        </div>

        <div class="chart-block meters">
        <h1>Pollution indices</h1>
        <img
        src="${drawPollutionMeters}"
        alt="HPI HEI Cd pollution index meters"
        />
        </div>
        </div>
        </section>
        ...
        <section class="page">
        <div class="section-label">SPATIAL &amp; TEMPORAL</div>

        <div class="chart-block map-chart">
        <h1>Pollution map</h1>
        <img
        src="${drawPollutionMap}"
        alt="Pollution map"
        />
        </div>

        <div class="chart-block">
        <h1>Temporal trend analysis</h1>
        <img
        src="${drawTemporalTrend}"
        alt="Temporal trend analysis chart"
        />
        </div>
        </section>
